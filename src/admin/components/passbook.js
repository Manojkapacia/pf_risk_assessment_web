...
const PFPassbook = ({ jsonData, onBack }) => {
    ...
    return (
        ...
        <div>
            ...
            <div className="d-flex mb-4" style={{ gap: "1rem" }}>
                ...
                >
                    ...
                    {data?.serviceHistory.history.map((historyItem, index) => {
                        ...
                        const company = historyItem.company;
                        const joiningDate = historyItem.details["Joining Date"];
                        const endDate = historyItem.details["Exit Date"] || "Present"; 
                ...
                {companyHasPassbook ? (
                    ...
                    >
                        <option value="">Select Year</option>
                        {Object.keys(data.passbooks[selectedCompany] || {})
                            .sort((a, b) => b - a)
                            .map((year, index) => (
                                <option key={index} value={year}>{year}</option>
                            ))}
            ...
            {selectedCompany && !companyHasPassbook && (
                <h6 className="text-danger">
                    No Passbook Data Found!! Either this is a Trust OR Transferred Service History
                </h6>
            )}

            {companyHasPassbook && selectedYear && isPassbookEmpty && (
                    ...
                    <div className="accordion" id="passbookAccordion">
                        <div className="accordion-body">
                            ...
                            <table className="table table-hover table-bordered text-center">
                                ...
                                <tbody>
                                    {passbookData?.transactions?.map((txn, index) => (
                                        <tr key={index}>
                                            <td>{txn?.wageMonth}</td>
                                            <td>{txn?.transactionDate}</td>
                                            <td>{txn?.transactionType}</td>
